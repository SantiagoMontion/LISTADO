...
import { listImportadosOrders } from '../_lib/importados-sync/importadosOrders.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    const orders = await listImportadosOrders()
    sendJson(res, 200, {
      ok: true,
      orders,
      count: orders.length,
      units: orders.reduce((sum, o) => sum + o.allSupplierUrls.length, 0),
    })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    console.error('[importados-sync/orders]', message)
